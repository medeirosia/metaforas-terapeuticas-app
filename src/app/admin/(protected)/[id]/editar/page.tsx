...
export default async function EditarMetaforaPage({
  ...
  const [{ data: categorias }, { data: metafora }] = await Promise.all([
    ...
    supabase
      .from("metaforas")
      .select(
        "titulo, status, video_url, thumb_url, resumo, descricao, categoria_id, dores, ficha_dor, ficha_usar, ficha_nao_usar, ficha_preparo, ficha_perguntas, ficha_ponte, destaque, mais_vistos, publicado, ordem"
      )
      .eq("id", id)
      .maybeSingle(),
  ]);
